...
async function connectToEc2Browser(): Promise<Browser> {
...
async function getPageForTest(browser: Browser): Promise<Page> {
  const contexts = browser.contexts();
  const context = contexts.length > 0 ? contexts[0] : await browser.newContext();
  const pages = context.pages();
  let page = pages.length > 0 ? pages[0] : await context.newPage();

  const targetUrl = 'http://ec2-13-61-208-154.eu-north-1.compute.amazonaws.com/audience-batches';
  ...
  if (currentUrl !== targetUrl) {
    ...
    console.log('[REPORT] Page already on audience-batches page, skipping navigation');
  }
  return page;
}
...
test.describe('Batch Validation - Group 1', () => {
  ...
  test.beforeAll(async () => {
    ...
    const page = await getPageForTest(sharedBrowser);
    savedCookies = await page.context().cookies();
    console.log('[REPORT] Cookies saved to preserve session');
  });

  // Before each test - prepare page and restore session
  test.beforeEach(async () => {
    ...
    await currentPage.context().addCookies(savedCookies);
    console.log('[REPORT] ✓ Cookies restored, page prepared and ready');
  });
...
test.describe('Batch Validation - Group 2', () => {
  ...
  scenarios.slice(10, 18).forEach((scenario, index) => {
